    ...
    if @dialog_id > 0
      ...
      for metamap in @metamaps
        if params["posted_by_metamap_#{metamap.id}"].to_i != 0
          ...
          @items = @items.joins("inner join metamap_node_participants p_mnp_#{metamap.id} on (p_mnp_#{metamap.id}.participant_id=#{item.posted_by} and p_mnp_#{metamap.id}.metamap_id=#{metamap.id} and p_mnp_#{metamap.id}.metamap_node_id=#{posted_by_metamap_node_id})")
